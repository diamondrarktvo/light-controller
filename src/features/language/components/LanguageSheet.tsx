...
import { BottomSheet, Box, Button, Image, Row, Text, TouchableOpacity } from '_shared';
...
const LanguageSheet = React.forwardRef<BottomSheetModal, LanguageSheetProps>((props, ref) => {
   const {
      t,
   ...
   return (
      <BottomSheet ref={ref} snapPoints={[1, '24%']}>
         ...
         {ALL_LANGUAGE_SUPPORTED.map(language => (
            <TouchableOpacity key={language.id} onPress={() => handleChangeLanguage(language.code)}>
               <Row
