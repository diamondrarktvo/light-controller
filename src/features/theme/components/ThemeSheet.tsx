...
import { BottomSheet, Button, Image, Row, Text, TouchableOpacity } from '_shared';
...
const ThemeSheet = React.forwardRef<BottomSheetModal, ThemeSheetProps>((props, ref) => {
   ...
   return (
      <BottomSheet ref={ref} snapPoints={[1, '24%']}>
         ...
         {ALL_THEME_SUPPORTED.map(theme => (
            <TouchableOpacity key={theme.id} onPress={() => handleChangeTheme(theme.code)}>
               <Row
                  ...
                  <Text
                     ...
                     color={selected === theme.code ? 'primary' : 'textBottomSheet'}>
                     {theme.label}
                  </Text>
